...
## High-Win Focused Update

User target added after the combined-pool run:
- win rate must be above 50%;
- return must be meaningfully larger than drawdown;
- backtest speed must be practical on local cached data;
- strategy selection should be based on a market thesis, not blind parameter mixing.

Research interpretation:
- The broad TSMOM family is still theoretically valid, but in this local sample it often improves return at the cost of lower win rate and larger drawdown.
- Daily Donchian/ATR and selective volatility breakout fit the "large trend, fewer trades" goal better for the small account.
- Short-only `trend_failure` is useful for selected intraday contracts, especially when the market fails to continue after a rebound, but it must pass fixed-strategy validation before live use.
- `confirmed_breakout` with volume/momentum confirmation was added and tested, but it did not qualify as a standalone live strategy. It stays in the research pool.

Implementation changes:
- `walk-forward` default candidate cap changed from 24 to 12 per symbol/timeframe/pattern, after research-prior ordering.
- Daily high-win Donchian/ATR variants are ordered first instead of expanding a large Cartesian grid.
- Active validation now requires weighted win rate above 50%, where win rate is weighted by trade count; no-trade windows no longer count as 0% win-rate windows.
- Train-window scoring now penalizes low win rate and drawdown more strongly.

Focused validation command:

```powershell
qihuo walk-forward --pattern donchian_atr --pattern vol_breakout --pattern trend_failure --timeframe 1d --timeframe 60m --timeframe 15m --max-per-symbol 12 --workers 8 --no-profile-write --reports-root reports/high_win_focused_walk_forward
```

Walk-forward active rows:

| Symbol | Pattern | Timeframe | Test PnL | Worst DD | Positive Windows | Weighted Win Rate | Trades |
| --- | --- | --- | ---: | ---: | ---: | ---: | ---: |
| RB | `donchian_atr` | 1d | 12,936.55 | -6,209.83 | 4/4 | 52.2% | 46 |
| SR | `vol_breakout` | 1d | 9,668.44 | -3,693.78 | 4/4 | 69.2% | 13 |
| C | `donchian_atr` | 60m | 1,490.00 | -589.10 | 4/4 | 61.5% | 13 |
| CS | `trend_failure` | 15m | 1,237.45 | -512.70 | 3/4 | 58.3% | 12 |

Fixed strategy validation:

The live profile stores one fixed `strategy_id`, so the selected live strategy was also re-run unchanged across the same four out-of-sample windows. Result file: `reports/fixed_strategy_validation.csv`.

| Symbol | Pattern | Timeframe | Fixed PnL | Worst DD | Positive Windows | Weighted Win Rate | Trades | Pass |
| --- | --- | --- | ---: | ---: | ---: | ---: | ---: | --- |
| SR | `vol_breakout` | 1d | 16,039.40 | -1,991.68 | 4/4 | 81.8% | 11 | yes |
| RB | `donchian_atr` | 1d | 12,478.89 | -5,215.19 | 3/4 | 56.4% | 39 | yes |
| C | `donchian_atr` | 60m | 1,540.00 | -589.10 | 4/4 | 69.2% | 13 | yes |
| CS | `trend_failure` | 15m | 851.08 | -512.70 | 3/4 | 71.4% | 7 | yes |

Current live decision:
- Promote SR, RB, C, and CS to `active`.
- Keep RM in `observe`. RM can pass a rolling selector when `quality_tsmom` is available for one early window, but the fixed live short strategy alone does not meet the reward/drawdown gate in the focused validation.
- Keep HC in `observe`. HC still has attractive large-trend windows, but positive windows are inconsistent and drawdown is too large for this account.
- Keep other symbols in `observe` until they pass both walk-forward and fixed-strategy validation.
