...
def main(argv: list[str] | None = None) -> int:
    ...
    fetch_news_p.add_argument("--dry-run", action="store_true", help="Print alert instead of calling QQ API")
    fetch_news_p.add_argument("--include-empty", action="store_true", help="Push a no-impact message when no events are classified")
    fetch_news_p.add_argument("--major-limit", type=int, default=30)

    news_poll_p = sub.add_parser("news-poll", help="Fetch real-time news, classify impacts, and push QQ alert")
    ...
    news_poll_p.add_argument("--dry-run", action="store_true", help="Print alert instead of calling QQ API")
    news_poll_p.add_argument("--include-empty", action="store_true", default=True)
    news_poll_p.add_argument("--major-limit", type=int, default=30)
